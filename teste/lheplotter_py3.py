...
from math import *
from ROOT import TFile, TH1D, TH2D, TLorentzVector, TPaveText, TLegend, gStyle, TCanvas, THStack, gPad
...
# INPUT FILES:
FILES   = [
"newaaaa_unweighted_events.lhe",
"newppaa_unweighted_events.lhe"
]; #FIXME
...
histoslog_axis   = ["M(x^{+}x^{-})","p_{T}(x^{#pm})","p_{T}(x^{+}x^{-})","#eta(x^{+}x^{-})","#phi(x^{+},x^{-})","E(x^{+},x^{-})","#Delta p_{T}(x^{+}x^{-})","1-|#Delta#phi(x^{+}x^{-})/#pi|","1-|#Delta#phi(x^{+}x^{-})/#pi|","|#Delta#phi(x^{+}x^{-})|","|#Delta#phi(x^{+}x^{-})|"];
...
for i in range(len(FILES)):
    ...
    pt_decay.append(TH1D("1D_pt_decay"+"_"+PDF[i]    , "", 50,  0., 50.));
    ptsum_decay.append(TH1D("1D_ptsum_decay"+"_"+PDF[i], "", 50,  0., 100.));
    eta_decay.append(TH1D("1D_eta_decay"+"_"+PDF[i]  , "", 50,-10.,  10.));
    phi_decay.append(TH1D("1D_phi_decay"+"_"+PDF[i], "", 10, -4.,   4.));
    E_decay.append(TH1D("1D_E_decay"+"_"+PDF[i], "", 50,  0., 100.));
    ...
    for line in f:
            ...
            elif coll[0] == '22' and coll[1] == '1' and ngamma == 0:
                    dp = TLorentzVector();
                    px = float(coll[6]);
                    py = float(coll[7]);
                    pz = float(coll[8]);
